...
RSpec.describe Profile, type: :model do
  describe 'validations' do
    it 'is valid with name and headline' do
      profile = build(:profile, name: 'Eduar Hernández', headline: 'Senior Ruby Developer')
      expect(profile).to be_valid
    end

    it 'is invalid without a name' do
      profile = build(:profile, name: nil, headline: 'Senior Ruby Developer')
      expect(profile).not_to be_valid
      expect(profile.errors[:name]).to include("can't be blank")
    end

    it 'is invalid without a headline' do
      profile = build(:profile, name: 'Eduar', headline: nil)
      expect(profile).not_to be_valid
      expect(profile.errors[:headline]).to include("can't be blank")
    end

    it 'is valid with correct email format' do
      profile = build(:profile, name: 'Eduar', headline: 'Dev', email: 'test@example.com')
      expect(profile).to be_valid
    end

    it 'is invalid with incorrect email format' do
      profile = build(:profile, name: 'Eduar', headline: 'Dev', email: 'invalid-email')
      expect(profile).not_to be_valid
  ...
  describe 'singleton rule' do
    before do
      create(:profile, name: 'Eduar Hernández', headline: 'Senior Ruby Developer')
    end

    it 'does not allow creating a second profile' do
      second_profile = build(:profile, name: 'Another', headline: 'Another Dev')
    ...
    it 'raises a database-level error when trying to bypass validations to insert a second profile' do
      expect {
        build(:profile, name: 'Another', headline: 'Another Dev').save(validate: false)
